  ...
  return (
    <section className="mb-5">
      ...
      <div className="flex flex-col gap-3">
        {items.map((item) => {
          const isReply = item.kind === "support_ticket_reply";
          return (
            <article key={item.id} className={`card relative overflow-hidden p-4 sm:p-5 ${isReply ? "border-accent/30" : "border-green/30"}`}>
              {item.read_at === null && <span className="absolute inset-y-0 left-0 w-1 bg-accent" aria-label="Unread" />}
              <div className="flex items-start gap-3">
                <span className={`grid h-9 w-9 shrink-0 place-items-center rounded-full ${isReply ? "bg-accent/12 text-accent-soft" : "bg-green/12 text-green"}`}>
                  {isReply ? <BellIcon className="h-[17px] w-[17px]" /> : <CheckIcon className="h-[17px] w-[17px]" />}
                </span>
                <div className="min-w-0 flex-1">
                  <div className="flex flex-wrap items-start justify-between gap-2">
                    <div>
                      ...
                      {!isReply && item.expires_at && <p className="mt-1.5 text-[11.5px] text-rose">Transcript deletion: {ticketDate(item.expires_at)}</p>}
                    </div>
                  ...
                  <div className="mt-3 flex flex-wrap items-center gap-4">
                    ...
                    <Link href={`/support/tickets/${item.ticket_id}`} className="text-[12px] font-semibold text-accent-soft hover:underline">{isReply ? "Open reply" : "Read transcript"}</Link>
                  </div>
                </div>
              </div>
            </article>
